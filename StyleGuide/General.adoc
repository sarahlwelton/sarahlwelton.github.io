...
* If it's a single word that must have specific capitalization, add it to the `Vocab\Couchbase\accept.txt` file.
* If it's a phrase or product name that contains multiple words that must be spelled or capitalized a specific way, add it to the `Couchbase\NamesAndTerms.yml` Vale Style. 
