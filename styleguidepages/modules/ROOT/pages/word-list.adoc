...
| one; 1,000; 1,000,000 | Write out all numbers less than 10, unless writing for xref:ui-ux:ROOT:index.adoc[UI and UX]. Use commas to separate every group of three digits in a large number. For more information about how to format numbers, see the https://developers.google.com/style/numbers#commas-and-decimal-points-in-numbers[Google Developer Style Guide].
...
| cURL | Write with a lowercase c and uppercase U, R, and L. Write as `curl` for the command line. 
|===
