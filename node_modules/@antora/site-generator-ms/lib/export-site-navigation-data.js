...
function exportSiteNavigationData (components, componentOrder = undefined, relativePath = 'site-navigation-data.js') {
  const navigationData = customSortComponents(components, componentOrder).map(({ name, title, versions }) => ({
    name,
    title,
    versions: versions
      .filter(({ site }) => !site)
      .map(({ version, displayVersion, navigation = [] }) => {
  ...
  return createSiteFile(relativePath, navigationDataSource, 'application/javascript')
}
